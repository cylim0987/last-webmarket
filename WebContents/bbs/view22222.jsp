...
<div class="container">
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-3">
            <input name="writer" class="form-control" value="${bbs.writer}" <c:if test="${sessionId=='admin'}"> readonly</c:if>>
        </div>
    </div>
        <div class="form-group row">
        <label class="col-sm-2 control-label">제목</label>
        <div class="col-sm-3">
            <input name="subject" class="form-control" value="${bbs.subject}" <c:if test="${sessionId=='admin'}"> readonly</c:if>>
        </div>
    </div>
    <div class="form-group row">
        ...
        <div class="col-sm-8" style="word-break:break-all;">
            <textarea rows="5" cols="50" name="content"
               class="form-control" <c:if test="${sessionId=='admin'}"> readonly</c:if>>${bbs.content}</textarea>
        </div>
    </div>
    ...
    <div class="form-group row">
        <div class="col-sm-offset-2 col-sm-10">
         ...
         <c:if test="${sessionId=='admin'}">
        	<button type="button" class="btn btn-dangeSr" data-toggle="modal" data-target="#exampleModal">삭제
        	</button>
              <input type="submit" class="btn btn-success" value="수정">
          </c:if>   
       
             <c:if test="${sessionId=='null'}"> readonly</c:if>
