 ...
 public void insertBbs(BbsDTO bbs){
	 Connection conn = null;
		...
		if(bbs.getRef()==0) { //ref가 0이면 신규글 등록
			sql = "insert into qna(num,writer,category, subject,content,password,ip,ref,re_step,re_level) "
			...
			updateSql = "update qna set re_step=re_step+1 where ref=? and re_step > ?"; 
		...
		try { //신규글 등록 처리
			...
			if(bbs.getRef()==0) {
				...
				pstmt.setString(2, bbs.getCategory());
				pstmt.setString(3, bbs.getSubject());
				pstmt.setString(4, bbs.getContent());
				pstmt.setString(5, bbs.getPassword());
				pstmt.setString(6, bbs.getIp());
				//신규 등록 시 : 글번호 = ref, re_step=0, re_level=0
				pstmt.setInt(7, 0); //신규 등록 시 re_step=0
				pstmt.setInt(8, 0); //신규 등록 시 re_level=0
				...
				pstmt.setString(6, bbs.getIp());
				//신규 등록 시 : 글번호=ref, re_step=0, re_level=0
				pstmt.setInt(7, bbs.getRef());
				pstmt.setInt(8, bbs.getRe_step()+1); //댓글 등록 시 re_step = re_step+1
				pstmt.setInt(9, bbs.getRe_level()+1); //댓글 등록 시 re_level = re_level+1
	  ...
	  String sql="";
		  ...
		  if((items==null && text==null)||( items.length()==0 || text.length()==0)) {//검색 조건이 파라미터로 넘어오지 않은 경우
				sql = "select * "
				    ...
				    + "       qna.subject subject, "
				    + "       qna.content content, "
				    + "       qna.readcount readcount, "
				    + "       qna.password password, "
				    + "       qna.reg_date reg_date, "
				    + "       qna.ip ip, "
				    + "       qna.ref ref, "
				    + "       qna.re_step re_step, "
				    + "       qna.re_level re_level, "
				    + "       nvl(bbsgoodbad.good,0) good, "
				    + "       nvl(bbsgoodbad.bad,0) bad "
				    + "  from qna, bbsgoodbad "
				    + " where qna.num=bbsgoodbad.num(+) "
			...
			try {
				...
				while(rs.next()) {
					...
					bbs.setCategory(rs.getString(4));
					bbs.setSubject(rs.getString(5));
					bbs.setContent(rs.getString(6));
					bbs.setReadcount(rs.getInt(7));
					bbs.setPassword(rs.getString(8));
					bbs.setReg_date(rs.getString(9));
					bbs.setIp(rs.getString(10));
					bbs.setRef(rs.getInt(11));
					bbs.setRe_step(rs.getInt(12));
					bbs.setRe_level(rs.getInt(13));
					//좋아요, 싫어요 추가
					bbs.setGood(rs.getInt(14));
					bbs.setBad(rs.getInt(15));
...
public int getFirstNum() {
	 ...
	 Connection conn=null;
	  ...
	  String sql="select nvl(min(num),0) from qna ";	
	try {
		 //1.OracleDB 연결객체 생성
		 conn = DBConnectionOracle.getConnection();
		 pstmt = conn.prepareStatement(sql);
...
public int getLastNum() {
	 ...
	 Connection conn=null;
	  ...
	  String sql="select nvl(max(num),0) from qna ";	
	try {
		 //1.OracleDB 연결객체 생성
		 conn = DBConnectionOracle.getConnection();
		 pstmt = conn.prepareStatement(sql);
...
public void updateBbs(BbsDTO bbs) {
	...
	String sql = "update qna set writer=?,subject=?,content=?,ip=? where num=?";	//category 안넣어봤음		
...
public void deleteBbs(int num) {
	...
	String replyCountSql="select count(*) from qna where ref=? and re_step >0";
